...
Wind2d* get_wind2d(const dictd& params) {
    ...
    int wind_type = params.at("wind_type");

    if (wind_type == 0) { // Constant wind
        double v[] = {params.at("v_wind_x"), params.at("v_wind_y")};
        return new Wind2d_const(v);
    }
    if (wind_type == 1) { // With simple stream function
        return new Wind2d_stream(params.at("k_wind"), params.at("eps_wind"));
    }
